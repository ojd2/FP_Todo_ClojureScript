...
(defn todo-input-handler [ev]
  (if (= ENTER_KEY (.-keyCode ev))
    (add-todo (.-value (by-id "todo-input")))))

(defn add-todo-input-handler [ev]
    (add-todo (.-value (by-id "todo-input"))))

;;(defn remove-todo-handler [ev]
...
(defn add-event-listeners []
  (ev/listen (by-id "todo-input") "keypress" todo-input-handler)
  (ev/listen (by-id "add-todo") "click" add-todo-input-handler)
  (ev/listen (by-id "remove-todo") "click" clear-click-handler)
